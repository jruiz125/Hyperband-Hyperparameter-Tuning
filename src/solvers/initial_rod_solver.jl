...
if !@isdefined(FORCE_STANDALONE_EXECUTION)
    const FORCE_STANDALONE_EXECUTION = false
end
...
# Output Files
Generated in timestamped folders under `dataset/MATLAB code/Learning_Data_ClampedPinned_Rod_IK/Find Initial Rod Shape/`:
...
function initial_rod_solver(config::Union{ClampedRodConfig, Nothing} = nothing)
    try
        ...
        # Generate filename suffixes for consistent naming (same logic as clamp_fixed_rod_solver)
        # Format xp suffix for file naming (e.g., 0.2 -> X02, 0.6 -> X06, -0.2 -> XN02)
        ...
        println("✓ Generated filename suffixes: $xp_suffix, $yp_suffix\n")
        ...
        @mput matlab_project_root timestamp_str dataset_folder_name xp yp mode xp_suffix yp_suffix
        ...
        fprintf('Using project root: %s\\n', project_root);
               ...
               functions_subpath = fullfile(functions_path, 'Functions');        % Create timestamped DATASET folder for saving data and figures (using consistent timestamp)
        ...
        % Use xp_suffix and yp_suffix passed from Julia (don't regenerate)
        xp_str = xp_suffix;  % Use Julia-generated suffix
        yp_str = yp_suffix;  % Use Julia-generated suffix
        ...
        @mput L N EI x0 y0 theta alpha lambda xp yp mode slope Nkr Npsi epsilon save_figures xp_suffix yp_suffix
        ...
        @mput matlab_project_root dataset_folder_name timestamp_str xp yp mode xp_suffix yp_suffix
        mat"""
        fprintf('\\n');
        ...
        %save(['CLampedPinnedRod_sol_' num2str(i) '_mode_' num2str(INBVP.mode)_ ],'IN', 'OUT') ;
                    ...
                    % Use xp_suffix and yp_suffix passed from Julia (don't regenerate from INBVP values)
                    xp_str = xp_suffix;  % Use Julia-generated suffix
                    yp_str = yp_suffix;  % Use Julia-generated suffix                    % Ensure we save in the correct directory (dataset folder)
...
if (!@isdefined(__SOLVER_ALREADY_LOADED__) && !@isdefined(__PIPELINE_ALREADY_LOADED__) && 
    ...
    custom_config = create_config(xp = 0.5, yp = 0.0, mode = 2.0, save_figures = true)
