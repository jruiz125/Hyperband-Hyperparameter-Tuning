...
# =============================================================================
    ...
    if best_config === nothing || best_loss == Inf
        ...
        println("="^60)
        println("All configurations returned Inf loss. This suggests:")
        println("1. The UDE training is failing for all hyperparameter combinations")
        println("2. There might be an issue with the neural network setup or ODE solving")
        println("3. The hyperparameter ranges might be too restrictive")
        println("\nTrying a single configuration with verbose output...")
        
        # Try one configuration with verbose output to debug
        test_hp = sample_hyperparameters()
        println("\nTest configuration:")
        println("  n_hidden: $(test_hp.n_hidden)")
        println("  n_layers: $(test_hp.n_layers)")
        println("  activation: $(test_hp.activation == 1 ? "tanh" : test_hp.activation == 2 ? "relu" : "sigmoid")")
        
        test_result = train_ude_with_hyperparameters(test_hp, 1000, verbose=true)
        println("Test result loss: $(test_result.loss)")
        
        if test_result.loss == Inf
            println("\nThe training function is failing. Please check:")
            println("1. Neural network initialization")
            println("2. ODE problem setup")
            println("3. Optimization parameters")
            println("4. Sensealg compatibility")
        end
        
        error("Hyperband optimization failed - no valid configurations found")
    end
