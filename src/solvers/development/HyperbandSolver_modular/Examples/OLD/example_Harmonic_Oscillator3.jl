#=
The issue is that Zygote is having compilation errors with the ForwardDiffSensitivity(). Let's fix this by using a different sensitivity algorithm and addressing the core issues:
=#
...
const HYPERPARAMETER_SPACE = (
    ...
    adam_lr = (1e-3, 1e-1),        # Adjusted learning rates
    adam_beta1 = (0.8, 0.95),      # Adjusted beta1
    adam_beta2 = (0.9, 0.999),     # Adjusted beta2
    adam_iterations = (10, 2000), # More iterations minimum
...
# =============================================================================
# TRAINING FUNCTION WITH GIVEN HYPERPARAMETERS (CORRECTED)
# =============================================================================
...
function train_ude_with_hyperparameters(hp, resource_budget; verbose=false)
    try
        ...
        function predict(θ)
            try
                # Try without sensitivity first for speed
                local_prob = remake(prob_ude, p=θ)
                sol = solve(local_prob, Tsit5(), 
                           saveat=t_train,
                           abstol=1e-6, 
                           reltol=1e-6,
                           maxiters=1e5)
        ...
        adam_iters = max(50, min(hp.adam_iterations, round(Int, resource_budget * 0.8)))
        lbfgs_iters = max(10, min(hp.lbfgs_iterations, round(Int, resource_budget * 0.2)))
        
        # Early stopping if resource budget is too small
        if adam_iters < 50
            return (loss = 1e8, analytical_error = 1e8, parameters = nothing, 
        ...
        callback = function(state, l)
            ...
            # Early stopping if loss is too large
            if l > 1e10
                return true
            end
            return false
        ...
        # Use ForwardDiff for AD which is more stable for small networks
        ...
        try
            ...
            if verbose
                println("  ADAM optimization failed: $(typeof(e))")
            end
            return (loss = 1e8, analytical_error = 1e8, parameters = nothing, 
        ...
        if res1.objective >= 1e8
            return (loss = res1.objective, analytical_error = 1e8, parameters = nothing, 
                   iterations = length(losses), hyperparameters = hp, NN = nothing, st = nothing)
        end
        ...
        if lbfgs_iters > 0 && final_loss < 100  # Only refine if loss is reasonable
            ...
            try
                ...
                if res2.objective < final_loss  # Only update if improved
                    final_params = res2.u
                ...
                if verbose
                    println("  LBFGS refinement failed, keeping ADAM results")
                end
            end
        end
...
# First, try a simple test to ensure the setup works
println("\nTesting baseline configuration first...")
test_hp = ComponentVector(
    n_hidden = 8,
    n_layers = 2,
    activation = 1,  # tanh
    adam_lr = 0.01,
...
test_result = train_ude_with_hyperparameters(test_hp, 600, verbose=true)
println("Test result: loss = $(test_result.loss)")

if test_result.loss >= 1e8
    println("\nWARNING: Even simple configuration failed!")
    println("Debugging the issue...")
else
    println("\nBaseline test successful! Proceeding with Hyperband...")
end

# Run Hyperband with reduced maximum resource
println("\nStarting Hyperband search...")
println("Hyperparameter space dimensions: 10")
println("Maximum resource (iterations): 5000")

best_config, best_loss = HyperbandSolver.hyperband(
    hyperband_objective,
    get_random_config,
    5000,  # Reduced maximum iterations for faster testing
    η = 3
)

# Initialize best_hp variable
best_hp = nothing
...
if best_config === nothing || best_loss >= 1e8
    ...
    println("="^60)
    println("Best loss found: $best_loss")
    println("\nTrying a refined baseline configuration...")
    
    # Try a refined baseline configuration
    ...
    baseline_result = train_ude_with_hyperparameters(baseline_hp, 1800, verbose=true)
    
    if baseline_result.loss < 1e8
        println("\nBaseline succeeded with loss: $(baseline_result.loss)")
        best_hp = baseline_hp
        best_loss = baseline_result.loss
        best_config = [Float64(baseline_hp[i]) for i in 1:length(baseline_hp)]
    else
        println("\nEven refined baseline failed. Using default configuration.")
        best_hp = baseline_hp  # Use it anyway for the rest of the code
        best_loss = baseline_result.loss
    end
else
    ...
    println("\nHyperband found valid configuration with loss: $best_loss")
end

# Only proceed if we have valid hyperparameters
if !isnothing(best_hp)
    println("\n" * "="^60)
    ...
    if !isnothing(final_result.NN) && !isnothing(final_result.st) && final_result.loss < 1e8
        ...
        println("Final loss: $(final_result.loss)")
    end
else
    println("\nNo valid hyperparameters found - cannot proceed with final training")
end
...
Changed AD backend: Switched from AutoZygote() to AutoForwardDiff() which is more stable for small neural networks
Removed sensitivity algorithm from ODE solve: Using remake and solving without explicit sensitivity for the forward pass
Changed Inf to finite values: Using 1e8 instead of Inf to avoid numerical issues
Added pre-test: Tests a baseline configuration before running Hyperband
Better error handling: Properly initializes best_hp and handles failures gracefully
Reduced initial resource: Using 5000 instead of 100000 for faster testing
More robust LBFGS: Only runs LBFGS if ADAM achieved reasonable loss (<100)
Increased minimum iterations: Ensures at least 50 ADAM iterations
The main issue was the incompatibility between Zygote and ForwardDiffSensitivity. By switching to AutoForwardDiff and removing the explicit sensitivity algorithm, the training should now work properly.
=#