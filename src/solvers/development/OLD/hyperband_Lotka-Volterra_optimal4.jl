...
function predict_ude(nn_model, p_trained, st, solver, tspan, t_save)
    ...
    # If still incomplete, use what we have and interpolate/extrapolate carefully
    if length(sol.t) < length(t_save)
        println("  ⚠️ Still incomplete ($(length(sol.t)) points). Using available data...")
        # Return only the points we successfully computed
        actual_t = sol.t
        actual_X = Array(sol)
...
X_pred_hb, t_hb = predict_ude(nn_hb, p_hb, st_hb, solver_hb, tspan_pred, t_pred)

println("Generating Random Search predictions...")
X_pred_rs, t_rs = predict_ude(nn_rs, p_rs, st_rs, solver_rs, tspan_pred, t_pred)

# Find the common time range
actual_points_hb = size(X_pred_hb, 2)
actual_points_rs = size(X_pred_rs, 2)
actual_points = min(actual_points_hb, actual_points_rs)

println("\nActual prediction points: HB=$actual_points_hb, RS=$actual_points_rs")

# Adjust all arrays to the same size (use the minimum common range)
if actual_points < 80
    println("⚠️ Adjusting to $actual_points common points...")
    
    # Use only the common time points
...
if !isempty(test_idx) && !isnan(extrap_error_hb)
    println("\n  Extrapolation region (t=3 to $(t_subset[end])):")
