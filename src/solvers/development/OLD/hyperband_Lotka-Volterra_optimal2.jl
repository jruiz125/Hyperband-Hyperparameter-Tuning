...
function create_trained_ude(config, training_iterations=1000; verbose=false)
    ...
    function predict(θ)
        ...
        sol = solve(_prob, solver, saveat=t_train,
                    ...
                    maxiters=10000,  # Increased max iterations
...
"""
Make predictions with trained UDE model (with error handling)
"""
function predict_ude(nn_model, p_trained, st, solver, tspan, t_save)
    ...
    sol = solve(prob, solver, saveat=t_save, 
                abstol=1e-6, reltol=1e-6, maxiters=100000)
    
    # If failed, try with a more robust solver
    if sol.retcode != :Success
        println("  ⚠️ Original solver failed, switching to Rodas5...")
        sol = solve(prob, Rodas5(), saveat=t_save, 
                    abstol=1e-6, reltol=1e-6, maxiters=100000)
    end
    
    # Check if we got all the requested time points
    if length(sol.t) != length(t_save)
        println("  ⚠️ Warning: Only got $(length(sol.t))/$(length(t_save)) time points")
        # Interpolate to get all points
        return hcat([sol(t) for t in t_save]...)
    end
...
if size(X_pred_hb, 2) < 80 || size(X_pred_rs, 2) < 80
    println("⚠️ Prediction arrays are incomplete. Adjusting analysis...")
    n_points = min(size(X_pred_hb, 2), size(X_pred_rs, 2), 80)
    X_pred_hb = X_pred_hb[:, 1:n_points]
    X_pred_rs = X_pred_rs[:, 1:n_points]
    X_true_subset = X_true_full[:, 1:n_points]
    t_subset = t_pred[1:n_points]
    
    # Adjust indices
    train_idx = 1:min(40, n_points)
    test_idx = (min(40, n_points)+1):n_points
else
    X_true_subset = X_true_full
    t_subset = t_pred
    train_idx = 1:40
    test_idx = 41:80
end
...
if !isnan(train_error_hb) && !isnan(train_error_rs) && train_error_hb > 0
    println("    Improvement: $(round(train_error_rs/train_error_hb, digits=2))x")
end

if !isnan(extrap_error_hb) && !isnan(extrap_error_rs)
...
# Mark training/extrapolation boundary if we have enough points
if size(X_true_subset, 2) >= 40
    scatter!(p1, [X_true_subset[1, 40]], [X_true_subset[2, 40]], 
...
summary = DataFrame(
    ...
    Improvement = [
        ...
        isnan(extrap_error_rs) || isnan(extrap_error_hb) ? "N/A" : extrap_error_rs/extrap_error_hb,
        r2_hb/r2_rs,
        "-",
        "-"
    ]
