...
function create_trained_ude(config, training_iterations=1000; verbose=false)
    ...
    function predict(θ)
        ...
        if sol.retcode != :Success
            ...
            sol = solve(_prob, Rodas5(), saveat=t_train,
                       abstol=1e-6, reltol=1e-6,
                       maxiters=10000,
                       sensealg=QuadratureAdjoint(autojacvec=ReverseDiffVJP(true)))
        end
...
function predict_ude(nn_model, p_trained, st, solver, tspan, t_save)
    ...
    # If still incomplete, interpolate
    if length(sol.t) < length(t_save)
        println("  ⚠️ Still incomplete ($(length(sol.t)) points). Using interpolation...")
        result = zeros(Float32, 2, length(t_save))
        for (i, t) in enumerate(t_save)
            if t <= sol.t[end]
                result[:, i] = sol(t)
            else
                # Extrapolate using last known point (not ideal but prevents crash)
                result[:, i] = sol.u[end]
            end
        end
        return result
...
# Verify dimensions and adjust if necessary
actual_points_hb = size(X_pred_hb, 2)
actual_points_rs = size(X_pred_rs, 2)
actual_points = min(actual_points_hb, actual_points_rs, 80)

if actual_points < 80
    println("\n⚠️ Warning: Predictions incomplete (HB: $actual_points_hb, RS: $actual_points_rs points)")
    println("   Adjusting analysis to available data...")
    
    # Adjust arrays to match actual points
    X_pred_hb = X_pred_hb[:, 1:actual_points]
    X_pred_rs = X_pred_rs[:, 1:actual_points]
    X_true_subset = X_true_full[:, 1:actual_points]
    t_subset = t_pred[1:actual_points]
    
    # Determine train/test split based on available data
    train_end_idx = min(40, actual_points)
    train_idx = 1:train_end_idx
    test_idx = (train_end_idx + 1):actual_points
else
...
if train_end_idx <= actual_points
...
summary = DataFrame(
    ...
    Improvement = Any[
        ...
        isnan(train_error_rs) || isnan(train_error_hb) ? "N/A" : train_error_rs/train_error_hb,
